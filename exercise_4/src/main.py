...
def load_seeds(path="../data/seeds_dataset.txt"):
    ...
    feature_names = [
        "Area A", "Perimeter P", "Compactness C",
...
def load_mushroom(path="../data/agaricus-lepiota.txt"):
    """
    Mushroom dataset: first column is class (e=edible, p=poisonous), remaining are categorical features.
    We return DataFrame (X_df) and y (0/1) and list of original feature names.
    """
    column_names = [
        "class",
        "cap-shape",
...
# -------------------------
def train_and_evaluate_classifiers(X, y, feature_names, dataset_name="dataset", categorical=False):
    """
    ...
    X_train_raw, X_test_raw, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=RANDOM_STATE, stratify=y)

    # Pipelines:
    # Decision Tree: we can feed numeric arrays. For categorical mushroom we convert to ordinal or one-hot.
    if categorical:
        # For Decision Tree, ordinal encoding is fine; for SVM we will one-hot + scaling.
        ordinal = ColumnTransformer([("ord", OrdinalEncoder(handle_unknown="use_encoded_value", unknown_value=-1), list(range(X_train_raw.shape[1])) )])
    ...
    results[dt_key] = {"model": dt, "report": report_dt, "confusion_matrix": cm_dt, "accuracy": acc_dt}
    ...
    if categorical:
        ...
        preproc = ColumnTransformer([
            ("ohe", OneHotEncoder(handle_unknown="ignore", sparse_output=False), list(range(X_train_raw.shape[1])))
        ])
        svc_pipeline = Pipeline([
            ("ohe", preproc),
            ("scaler", StandardScaler(with_mean=False)),  # with_mean=False because sparse->dense; ok here
    ...
    param_grid = [
        ...
        {"svc__kernel": ["poly"], "svc__C": [1], "svc__degree":[2,3], "svc__gamma":["scale"]},
    ...
    if not best_svc:
        gs = GridSearchCV(svc_pipeline, param_grid, cv=4, scoring="accuracy", n_jobs=-1, verbose=0)
    ...
    results["svc_full"] = {"model": best_svc, "report": report_svc, "confusion_matrix": cm_svc, "accuracy": acc_svc, "grid_best_params": svc_params}
    ...
    print_and_save_report("Decision Tree (full features) accuracy: {:.4f}".format(acc_dt), out_file)
    print_and_save_report("Decision Tree classification report:\n" + report_dt, out_file)
    print_and_save_report("Decision Tree confusion matrix:\n" + str(cm_dt), out_file)

    print_and_save_report("SVM (best via gridsearch) accuracy: {:.4f}".format(acc_svc), out_file)
...
def aggregate_importances_for_categorical(importances_encoded, X_df):
    """
    If the tree was trained on OneHot encoded columns, but we originally had categorical columns,
    we may want to sum importances by original categorical feature. For our pipeline we used
    ordinal for DT on mushroom, so this function might not be needed. We'll leave a helper stub.
    """
    # not used in current pipeline version; kept for completeness
    return importances_encoded

def visualize_decision_boundary_2features_for_model(clf, X_full, y, top2_indices, feature_names, title="Decision boundary"):
    """
    ...
    mesh_step = 0.02
    xx, yy = np.meshgrid(np.arange(min_x, max_x, mesh_step), np.arange(min_y, max_y, mesh_step))
...
def pick_top2_features(importances, feature_names, categorical=False, X_df=None):
    ...
    For categorical with one-hot encoding, you'd want to aggregate; here we assume importances length == len(feature_names).
    """
    idx = np.argsort(importances)[-2:][::-1]
    return list(idx)

def get_make_pipeline():
    return make_pipeline(StandardScaler(), SVC(kernel="rbf", C=1.0, gamma="scale", random_state=RANDOM_STATE))
...
def save_models(results, dataset_name="dataset", output_dir=MODELS_DIR):
    ...
    for key, val in results.items():
        ...
        if key == "svc_full":
            ...
            file_path = os.path.join(output_dir, f"{dataset_name}_{key}_grid_best_params.joblib")
...
def load_model(key, dataset_name, output_dir=MODELS_DIR):
    ...
    if os.path.exists(file_path):
        ...
        if key == 'svc_full':
            params_file_path = os.path.join(output_dir, f"{dataset_name}_{key}_grid_best_params.joblib")
...
def run_all():
    ...
    results_seeds, imp_seeds, X_train_s_raw, X_test_s_raw, y_train_s, y_test_s, fn_seeds = train_and_evaluate_classifiers(
    ...
    print("Seeds top2 feature indices:", top2_seeds, "names:", [feature_names_seeds[i] for i in top2_seeds])
    ...
    visualize_decision_boundary_2features_for_model(dt2, np.vstack([X2_train, X2_test]), np.hstack([y_train_s, y_test_s]), [0,1], [feature_names_seeds[i] for i in top2_seeds], title=f"Seeds DT on top2: {feature_names_seeds[top2_seeds[0]]} vs {feature_names_seeds[top2_seeds[1]]}")

    # also show SVM behaviour on these two features (train simple scaled SVM)
    svc2 = get_make_pipeline()
    svc2.fit(X2_train, y_train_s)
    visualize_decision_boundary_2features_for_model(svc2, np.vstack([X2_train, X2_test]), np.hstack([y_train_s, y_test_s]), [0,1], [feature_names_seeds[i] for i in top2_seeds], title=f"Seeds SVM (RBF) on top2")

    # --- MUSHROOM dataset ---
    X_mush_df, y_mush, feature_names_mush = load_mushroom()
    results_mush, imp_mush, X_train_m_raw, X_test_m_raw, y_train_m, y_test_m, fn_mush = train_and_evaluate_classifiers(
        X_mush_df, y_mush, feature_names_mush, dataset_name="mushroom", categorical=True
    )

    save_models(results_mush, dataset_name="mushroom")

    # For mushroom, DecisionTree.importances_ correspond to ordinal-encoded columns (one per original feature),
    # so picking top2 is straightforward (we trained DT with ordinal encoder)
    top2_mush = pick_top2_features(imp_mush, feature_names_mush)
    print("Mushroom top2 feature indices:", top2_mush, "names:", [feature_names_mush[i] for i in top2_mush])

    # Build simple visualization models trained only on those two original features.
    # For mushroom, extract the two categorical columns, ordinal-encode them to numeric for plotting
    oe = OrdinalEncoder(handle_unknown="use_encoded_value", unknown_value=-1)
    X_mush_2cols = oe.fit_transform(X_mush_df.iloc[:, top2_mush])
    Xm_train2, Xm_test2, ym_train2, ym_test2 = train_test_split(X_mush_2cols, y_mush, test_size=0.25, random_state=RANDOM_STATE, stratify=y_mush)

    dt_m2 = DecisionTreeClassifier(max_depth=8, random_state=RANDOM_STATE)
    dt_m2.fit(Xm_train2, ym_train2)
    visualize_decision_boundary_2features_for_model(dt_m2, np.vstack([Xm_train2, Xm_test2]), np.hstack([ym_train2, ym_test2]), [0,1], [feature_names_mush[i] for i in top2_mush], title=f"Mushroom DT on top2 categorical (ordinal enc)")

    svc_m2 = get_make_pipeline()
    svc_m2.fit(Xm_train2, ym_train2)
    visualize_decision_boundary_2features_for_model(svc_m2, np.vstack([Xm_train2, Xm_test2]), np.hstack([ym_train2, ym_test2]), [0,1], [feature_names_mush[i] for i in top2_mush], title=f"Mushroom SVM (RBF) on top2 (ordinal enc)")

    # -------------------------
    # 7) Kernel summary:
    # -------------------------
    summary_lines = []
    summary_lines.append("SVM Kernel summary (simple conclusions based on GridSearch metrics):\n")
    summary_lines.append("Seeds dataset: best SVM params: " + str(results_seeds["svc_full"]["grid_best_params"]) + "\n")
    summary_lines.append("Seeds SVM accuracy: {:.4f}\n".format(results_seeds["svc_full"]["accuracy"]))
    summary_lines.append("Mushroom dataset: best SVM params: " + str(results_mush["svc_full"]["grid_best_params"]) + "\n")
    summary_lines.append("Mushroom SVM accuracy: {:.4f}\n".format(results_mush["svc_full"]["accuracy"]))

    summary_lines.append("\nShort notes on kernels (to include in repository README):")
    summary_lines.append("- linear: good for linearly separable data; simple; fewer hyperparams.")
    summary_lines.append("- rbf: flexible; maps to infinite-dim; often good default; gamma controls locality (large gamma -> more complex, risk overfit).")
    summary_lines.append("- poly: useful if polynomial relationships; degree controls complexity.")
    summary_lines.append("- sigmoid: rarely best; behaves like NN activation; use sparingly.")
    summary_text = "\n".join(summary_lines)
